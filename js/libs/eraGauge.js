eraGauge = function() {
    ...
    eraGauge.defaultSliderIndicator = function(g, w) {
        g.append("line")
            .attr("y1", -w/2)
            .attr("y2", w/4)
            .style("stroke", "red")
    ...
    eraGauge.arcslider = function()
    {
        ...
        {
            _range = eraGauge_scaleRange(axis.scale());
            _extent = eraGauge_scaleExtent(axis.scale());
            ...
            g.each(function() {
                ...
                bandsUpdate.attr("transform", "translate(" + (radius + margin.left) + ", " + (radius + margin.top) + ")")
                    .each(function(band) {d3.select(this).append("path")
                                ...
                                .attr("d", d3.svg.arc()
                                            .startAngle(eraGauge_convert(axis.scale(), band.domain[0]))
                                            .endAngle(eraGauge_convert(axis.scale(), band.domain[1]))
                ...
                redraw(eraGauge_invert(axis.scale(), _range[0]));
            });
        }
        ...
        function redraw(value, td)
        {
            ...
            var rad = eraGauge_convert(axis.scale(), value);
            ...
            _cursorUpdate.transition()
                ...
                .attrTween("d", function() {
                    ...
                    return function(step) {
                        ...
                        _currentValue = eraGauge_invert(axis.scale(), _currentRad);
        ...
        function radToValue(omega) {
            ...
            return eraGauge_invert(axis.scale(), min ? _range[0] : max ? _range[1] : omega);
        }
        return arcslider;
    };
    eraGauge.defaultArcSliderIndicator = function(g, r) {
        //g.append("circle").attr("r", 0.95 * r).attr("class", "knob");
        g.append("path").attr("d", "M0 " + -0.7 * r + " L 0 " + -0.98 * r + "");
    };
    eraGauge.defaultGaugeIndicator = function(g, r) {
        g.append("path").attr("d", "M0 " + 0.2 * r + " L 0 " + -1.05 * r + "");
        g.append("circle").attr("r", 0.03 * r);
    };
        
    eraGauge.arcaxis = function() {
        ...
        function arcaxis(g) {
            g.each(function() {
                ...
                var subticks = eraGauge_axisSubdivide(scale, ticks, tickSubdivide)
                ...
                var extent = eraGauge_scaleExtent(scale)
                ...
                var lineEnter = tickEnter.select("line")
                    ...
                    , text = tick.select("text").text(function(d,i){
                        return era_feature_mapping[Object.keys(era_feature_mapping)[i]];
                ...
                switch (orient) {
                    case "out":
                    {
                        tickTransform = eraGauge_axis_transform;
                        ...
                        if(normalize) {
                            textTransform = eraGauge_text_transform_normalize_out;
                    ...
                    case "in":
                    {
                        ...
                        if(normalize) {
                            textTransform = eraGauge_text_transform_normalize_in;
                            ...
                            text.attr("class", "unselectable").style("cursor","pointer");
                        }
                        else {
                            textEnter.attr("y", Math.max(tickMajorSize, 0) + tickPadding + t);
                            textUpdate.attr("x", 0).attr("y", Math.max(tickMajorSize, 0) + tickPadding + t);
                            text.attr("class", "unselectable").attr("dy", ".71em").style("text-anchor", "middle").style("cursor","pointer");
                        }
                        break;
                    }
                }
        ...
        arcaxis.orient = function(x) {
            ...
            orient = x in eraGauge_axisOrients ? x + "" : eraGauge_axisDefaultOrient;
            return arcaxis;
        };
    ...
    function eraGauge_extent(domain) {
        var start = domain[0], stop = domain[domain.length - 1];
        return start < stop ? [ start, stop ] : [ stop, start ];
    }
    function eraGauge_scaleExtent(scale) {
        return scale.rangeExtent ? scale.rangeExtent() : eraGauge_extent(scale.range());
    }
    function eraGauge_scaleRange(scale) {
        var extent = eraGauge_scaleExtent(scale);
        var range = scale.range();
        return range[0] < range[range.length - 1] ? [ extent[0], extent[1] ] : [ extent[1], extent[0] ];
    }
    function eraGauge_convert(scale, x) {
        var d = scale(x);
        isNaN(d) ? d = eraGauge_scaleRange(scale)[0] : d;
        return scale.rangeBand ? d + scale.rangeBand() / 2 : d;
    }
    function eraGauge_invert(scale, x) {
        if(scale.invert) return scale.invert(x);

        var l = scale.domain().length;
        var range = eraGauge_scaleRange(scale);
        var band = (range[1] - range[0]) / l;
        var index = Math.floor((x - range[0]) / band);
        return scale.domain()[index < l ? index : l-1];

    }
    var eraGauge_axisDefaultOrient = "out", eraGauge_axisOrients = {
        in: 1,
        out: 1
    };
    function eraGauge_axis_transform(selection, x, r) {
        selection.attr("transform", function(d) {
            return "translate(" + r * Math.sin(x(d)) + "," + -r * Math.cos(x(d)) + ") rotate("+ 180 / Math.PI * x(d) +")";
        });
    }
    function eraGauge_text_transform_normalize_out(selection, x, dr) {
        selection.attr("transform", function(d) {
    ...
    function eraGauge_text_transform_normalize_in(selection, x, dr, em) {
        selection.attr("transform", function(d) {
            var a = x(d) + (x.rangeBand ? x.rangeBand() / 2 : 0);
            return "rotate("+ -180 / Math.PI * a + ")" + "translate(" + -dr * Math.sin(a) + "," + dr * Math.cos(a) + ")";
            })
    ...
    function eraGauge_axisSubdivide(scale, ticks, m) {
        subticks = [];
        if (m && ticks.length > 1) {
            var extent = eraGauge_extent(scale.domain()), subticks, i = -1, n = ticks.length, d = (ticks[1] - ticks[0]) / ++m, j, v;
            while (++i < n) {
                for (j = m; --j > 0; ) {
