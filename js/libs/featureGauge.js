featureGauge = function() {
    var featureGauge = {
        version: "0.0.2"
    };
    
    featureGauge.defaultSliderIndicator = function(g, w) {
        g.append("line")
            .attr("y1", -w/2)
            .attr("y2", w/4)
            .style("stroke", "red")
    ...
    featureGauge.arcslider = function()
    {
        var radius = 100, minEventInterval = 100, enableFlick = true, transitionDuration = 600, arcFactor = 0.5, events = true, moveToTouch = true, ease = "elastic";
        var margin = {"top": 50, "left": 50, "bottom": 50, "right": 50};
        var axis = featureGauge.arcaxis()
        ...
        {
            _range = featureGauge_scaleRange(axis.scale());
            _extent = featureGauge_scaleExtent(axis.scale());
            ...
            g.each(function() {
                ...
                bandsUpdate.attr("transform", "translate(" + (radius + margin.left) + ", " + (radius + margin.top) + ")")
                    .each(function(band) {d3.select(this).append("path")
                                ...
                                .attr("d", d3.svg.arc()
                                            .startAngle(featureGauge_convert(axis.scale(), band.domain[0]))
                                            .endAngle(featureGauge_convert(axis.scale(), band.domain[1]))
                ...
                redraw(featureGauge_invert(axis.scale(), _range[0]));
            });
        }
        ...
        function redraw(value, td)
        {
            ...
            var rad = featureGauge_convert(axis.scale(), value);
            ...
            _cursorUpdate.transition()
                ...
                .attrTween("d", function() {
                    ...
                    return function(step) {
                        ...
                        _currentValue = featureGauge_invert(axis.scale(), _currentRad);
        ...
        function radToValue(omega) {
            ...
            return featureGauge_invert(axis.scale(), min ? _range[0] : max ? _range[1] : omega);
        }
        return arcslider;
    };
    featureGauge.defaultArcSliderIndicator = function(g, r) {
        //g.append("circle").attr("r", 0.95 * r).attr("class", "knob");
        g.append("path").attr("d", "M0 " + -0.7 * r + " L 0 " + -0.98 * r + "");
    };
    featureGauge.defaultGaugeIndicator = function(g, r) {
        g.append("path").attr("d", "M0 " + 0.2 * r + " L 0 " + -1.05 * r + "");
        g.append("circle").attr("r", 0.03 * r);
    };
        
    featureGauge.arcaxis = function() {
        ...
        function arcaxis(g) {
            g.each(function() {
                ...
                var subticks = featureGauge_axisSubdivide(scale, ticks, tickSubdivide)
                ...
                var extent = featureGauge_scaleExtent(scale)
                ...
                var lineEnter = tickEnter.select("line")
                    ...
                    , text = tick.select("text").text(function(d,i){
                        return Object.keys(feature_display_mapping)[i];
                    }).attr("fill",function(d,i){return i==0?"red":"white"})
                ...
                switch (orient) {
                    case "out":
                    {
                        tickTransform = featureGauge_axis_transform;
                        ...
                        if(normalize) {
                            textTransform = featureGauge_text_transform_normalize_out;
                            ...
                            text.attr("class", "unselectable").attr("dy", "0em").style("cursor","pointer");
                        }
                        else {
                            textEnter.attr("y", -(Math.max(tickMajorSize, 0) + tickPadding));
                            textUpdate.attr("x", 0).attr("y", -(Math.max(tickMajorSize, 0) + tickPadding));
                            text.attr("class", "unselectable").attr("dy", "0em").style("text-anchor", "middle").style("cursor","pointer");
                        }
                    ...
                    case "in":
                    {
                        ...
                        if(normalize) {
                            textTransform = featureGauge_text_transform_normalize_in;
        ...
        arcaxis.orient = function(x) {
            ...
            orient = x in featureGauge_axisOrients ? x + "" : featureGauge_axisDefaultOrient;
            return arcaxis;
        };
    ...
    function featureGauge_extent(domain) {
        var start = domain[0], stop = domain[domain.length - 1];
        return start < stop ? [ start, stop ] : [ stop, start ];
    }
    function featureGauge_scaleExtent(scale) {
        return scale.rangeExtent ? scale.rangeExtent() : featureGauge_extent(scale.range());
    }
    function featureGauge_scaleRange(scale) {
        var extent = featureGauge_scaleExtent(scale);
        var range = scale.range();
        return range[0] < range[range.length - 1] ? [ extent[0], extent[1] ] : [ extent[1], extent[0] ];
    }
    function featureGauge_convert(scale, x) {
        var d = scale(x);
        isNaN(d) ? d = featureGauge_scaleRange(scale)[0] : d;
        return scale.rangeBand ? d + scale.rangeBand() / 2 : d;
    }
    function featureGauge_invert(scale, x) {
        if(scale.invert) return scale.invert(x);

        var l = scale.domain().length;
        var range = featureGauge_scaleRange(scale);
        var band = (range[1] - range[0]) / l;
        var index = Math.floor((x - range[0]) / band);
        return scale.domain()[index < l ? index : l-1];

    }
    var featureGauge_axisDefaultOrient = "out", featureGauge_axisOrients = {
        in: 1,
        out: 1
    };
    function featureGauge_axis_transform(selection, x, r) {
        selection.attr("transform", function(d) {
            return "translate(" + r * Math.sin(x(d)) + "," + -r * Math.cos(x(d)) + ") rotate("+ 180 / Math.PI * x(d) +")";
        });
    }
    function featureGauge_text_transform_normalize_out(selection, x, dr) {
        selection.attr("transform", function(d) {
    ...
    function featureGauge_text_transform_normalize_in(selection, x, dr, em) {
        selection.attr("transform", function(d) {
            var a = x(d) + (x.rangeBand ? x.rangeBand() / 2 : 0);
            return "rotate("+ -180 / Math.PI * a + ")" + "translate(" + -dr * Math.sin(a) + "," + dr * Math.cos(a) + ")";
            })
    ...
    function featureGauge_axisSubdivide(scale, ticks, m) {
        subticks = [];
        if (m && ticks.length > 1) {
            var extent = featureGauge_extent(scale.domain()), subticks, i = -1, n = ticks.length, d = (ticks[1] - ticks[0]) / ++m, j, v;
            while (++i < n) {
                for (j = m; --j > 0; ) {
